...
public class DeferredFamilyRegistry<T extends ItemLike> extends Cache<Identifier, DeferredFamily<T>> implements FamilyFactory<T> {
    ...
    @Override
    public Family<T> create(Identifier name, CreativeModeTab group, TypeList types) {
        if (name == null) {
            return none;
        }
        return get(name);
