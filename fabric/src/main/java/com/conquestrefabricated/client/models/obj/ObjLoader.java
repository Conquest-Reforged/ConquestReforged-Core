...
import com.conquestrefabricated.client.models.obj.util.ModResourceReloader;
 ...
 */
public class ObjLoader extends ModResourceReloader implements UnbakedModelLoader<ObjModel> {
    ...
    public ObjGeometry loadGeometry(ObjGeometry.Settings settings) {
        return geometryCache.computeIfAbsent(settings, (data) -> {
            Resource resource = Minecraft.getInstance().getResourceManager().getResource(settings.modelLocation()).orElseThrow();
