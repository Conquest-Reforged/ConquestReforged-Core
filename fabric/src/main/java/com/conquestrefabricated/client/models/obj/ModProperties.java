...
/**
 * Properties for {@link BlockModel}s and {@link UnbakedModel}s.
 * Adapted from NeoForge's NeoForgeModelProperties (LGPL-2.1-only).
 */
public final class ModProperties {
    ...
    public static void fillRootTransformProperty(ContextMap.Builder propertiesBuilder, @Nullable Transformation rootTransform) {
        if (rootTransform != null) {
            propertiesBuilder.withParameter(ModProperties.TRANSFORM, rootTransform);
        }
    }
    ...
    public static void fillPartVisibilityProperty(ContextMap.Builder propertiesBuilder, Map<String, Boolean> partVisibility) {
        if (!partVisibility.isEmpty()) {
            Map<String, Boolean> visibility = propertiesBuilder.getOptionalParameter(ModProperties.PART_VISIBILITY);
            if (visibility != null) {
            ...
            propertiesBuilder.withParameter(ModProperties.PART_VISIBILITY, visibility);
        }
    }
}