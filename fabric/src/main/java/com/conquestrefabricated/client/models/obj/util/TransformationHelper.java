...
public class TransformationHelper {
        ...
        public Transformation deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            ...
            if (!elements.isEmpty()) {
                throw new JsonParseException(
                        "TRSR: can either have single 'matrix' key, or a combination of 'translation', 'rotation' OR 'left_rotation', 'scale', 'post-rotation' (legacy) OR 'right_rotation', 'origin'. Found: " + String.join(", ", elements));
            }
    ...
    public enum TransformOrigin implements StringRepresentable {
        ...
        public static @Nullable TransformOrigin fromString(String originName) {
            if (CENTER.getSerializedName().equals(originName)) return CENTER;
            if (CORNER.getSerializedName().equals(originName)) return CORNER;
            if (OPPOSING_CORNER.getSerializedName().equals(originName)) return OPPOSING_CORNER;
            return null;
        }
    }
}