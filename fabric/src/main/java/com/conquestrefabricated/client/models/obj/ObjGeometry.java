...
import com.conquestrefabricated.client.models.obj.ObjMaterialLibrary.Material;
...
public class ObjGeometry implements ExtendedUnbakedGeometry {
    ...
    public QuadCollection bake(TextureSlots textureSlots, ModelBaker baker, ModelState state, ModelDebugName debugName) {
        ...
        ModProperties.fillRootTransformProperty(propertiesBuilder, parameters.rootTransform());
        ModProperties.fillPartVisibilityProperty(propertiesBuilder, parameters.partVisibility());
        return bake(textureSlots, baker, state, debugName, propertiesBuilder.create(ModProperties.TYPE));
    }
    ...
    public QuadCollection bake(TextureSlots textureSlots, ModelBaker baker, ModelState state, ModelDebugName debugName, ContextMap additionalProperties) {
        Map<String, Boolean> partVisibility = additionalProperties.getOrDefault(ModProperties.PART_VISIBILITY, Map.of());
    ...
    private Pair<BakedQuad, Direction> makeQuad(ModelBaker baker, int[][] indices, int tintIndex, Vector4f colorTint, Vector4f ambientColor, Baked material, Transparency transparency, Transformation transform) {
        ...
        Direction cull = null;
        if (automaticCulling) {
            if (Mth.equal(pos[0].x(), 0) && Mth.equal(pos[1].x(), 0) && Mth.equal(pos[2].x(), 0) && Mth.equal(pos[3].x(), 0) && norm[0].x() < 0) {
                cull = Direction.WEST;
            } else if (Mth.equal(pos[0].x(), 1) && Mth.equal(pos[1].x(), 1) && Mth.equal(pos[2].x(), 1) && Mth.equal(pos[3].x(), 1) && norm[0].x() > 0) {
                cull = Direction.EAST;
            } else if (Mth.equal(pos[0].z(), 0) && Mth.equal(pos[1].z(), 0) && Mth.equal(pos[2].z(), 0) && Mth.equal(pos[3].z(), 0) && norm[0].z() < 0) {
                cull = Direction.NORTH;
            } else if (Mth.equal(pos[0].z(), 1) && Mth.equal(pos[1].z(), 1) && Mth.equal(pos[2].z(), 1) && Mth.equal(pos[3].z(), 1) && norm[0].z() > 0) {
                cull = Direction.SOUTH;
            } else if (Mth.equal(pos[0].y(), 0) && Mth.equal(pos[1].y(), 0) && Mth.equal(pos[2].y(), 0) && Mth.equal(pos[3].y(), 0) && norm[0].y() < 0) {
                cull = Direction.DOWN;
            } else if (Mth.equal(pos[0].y(), 1) && Mth.equal(pos[1].y(), 1) && Mth.equal(pos[2].y(), 1) && Mth.equal(pos[3].y(), 1) && norm[0].y() > 0) {
                cull = Direction.UP;
            }
        }
    ...
    private class ModelMesh {
        ...
        public void addQuads(QuadCollection.Builder builder, TextureSlots slots, ModelBaker baker, ModelState state, ModelDebugName debugName, ContextMap additionalProperties) {
            ...
            var rootTransform = additionalProperties.getOrDefault(ModProperties.TRANSFORM, Transformation.IDENTITY);
