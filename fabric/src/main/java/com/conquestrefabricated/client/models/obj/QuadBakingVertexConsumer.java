 ...
 * <p>
 * Note: unlike the original Create/NeoForge version, this does not store per-vertex normals on the baked quad
 * (that required a Mixin interface-injection onto vanilla's BakedQuad we chose not to reproduce blind). Quads
 * render with standard flat per-face shading instead of smooth interpolated normals.
 */
public class QuadBakingVertexConsumer implements VertexConsumer {
    ...
    public BakedQuad bakeQuad(Interner interner) {
        ...
        BakedQuad.MaterialInfo materialInfo = new BakedQuad.MaterialInfo(sprite, chunkLayer, itemRenderType, tintIndex, shade, lightEmission);
